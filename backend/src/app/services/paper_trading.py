...
Responsibilities:
    - Start/stop Freqtrade subprocess in dry-run mode
    - Authenticate with and proxy Freqtrade's REST API (port 8080)
    - Expose open trades, closed trades, profit, and performance data
    - Log trade events for persistence across restarts
...
class PaperTradingService:
    """Manages Freqtrade dry-run subprocess and proxies its API."""
    ...
    def __init__(self, log_dir: Path | None = None) -> None:
        ...
        # Freqtrade API config (read from config.json)
    ...
    def is_running(self) -> bool:
        """Check if Freqtrade subprocess is alive."""
    ...
    def stop(self) -> dict:
        """Gracefully stop the Freqtrade process (SIGTERM, then SIGKILL)."""
    ...
    def get_status(self) -> dict:
        ...
        if not self.is_running:
            ...
            if self._process is not None:
                ...
                self._process = None  # Clean up dead process reference
    ...
    async def _get_token(self) -> str | None:
        """Authenticate with Freqtrade API and get JWT token."""
        if self._jwt_token:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.post(
                    ...
                    data={
                        "username": self._ft_username,
                        "password": self._ft_password,
                    },
                    timeout=5.0,
                )
            ...
            logger.debug("Freqtrade API not reachable (process may be starting)")
        except Exception as e:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 401:
                    self._jwt_token = None  # Token expired, re-auth next call
        except httpx.ConnectError:
    ...
    async def get_freqtrade_config(self) -> dict:
        """Get running Freqtrade configuration."""
        data = await self._ft_get("show_config")
        return data if isinstance(data, dict) else {}

    # ── Event log ─────────────────────────────────────────────

    def _log_event(self, event: str, data: dict | None = None) -> None:
        """Append an event to the paper trading log (JSONL)."""
        entry = {
    ...
    def get_log_entries(self, limit: int = 100) -> list[dict]:
        """Read recent entries from the paper trading log."""
