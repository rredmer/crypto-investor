...
Provides regime detection and strategy recommendations for tracked symbols.
Uses the shared data pipeline to fetch OHLCV data per symbol.
"""
...
class RegimeService:
    """Wraps RegimeDetector + StrategyRouter for the dashboard API."""
    ...
    def get_recommendation(self, symbol: str) -> dict | None:
        """Get strategy recommendation for a symbol based on regime."""
        df = self._load_data(symbol)
        if df is None or df.empty:
            if symbol in self._cache:
